...
def getTransactionID():
    ...
    if(df is None):
        lista = {"TransactionID":[0], "Challenge":[random.randint(1,10)], "Seed":[" "], "Winner": [-1]}
        ...
        if(df.iloc[tam-1, 3] == -1):
            ...
            lista = {"TransactionID":transactionID, "Challenge":[random.randint(1,10)], "Seed":[" "], "Winner": [-1]}
...
def main():
    ...
    def eleicao(ch, method, properties, body):
        ...
        if(len(election) == qtd_usuarios):
            ...
            pa = json.loads(NodeId)

            if(pa.get("NodeId") == leader.get("NodeId")):
